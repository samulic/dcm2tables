...
def main(argv):
    ...
    if not os.path.exists(args.outputDirectory):
        create = raw_input(
            'Output directory does not exist. Would you like to create it? [y/n]:\n')
        if create.lower() in ["y", "yes", "true"]:
            ...
            if not create.lower() in ["", "n", "no", "false"]:
                logger.error(
                    "Entered letters %s unknown to this program." % create)
            return

    if not os.listdir(args.outputDirectory):
        ...
        for root, dirs, files in os.walk(args.inputDirectory):
            for f in files:
                ...
                try:
                    dicomParser = SRCDParser(
                        dcmName, tablesRules, tempPath=tempPath, dcmqiPath=args.dcmqiPath, logger=logger)
                except:
                ...
                for t in tables.keys():
                    if type(tables[t]) == pandas.DataFrame:
                        ...
                        if not os.path.isfile(fileName):
                            ...
                            tables[t].to_csv(
                                fileName, index=False, sep='\t', mode='a', header=False)

        pbar.close()
    else:
        logger.warning(
            f'output directory {args.outputDirectory} not empty, not re-running')
